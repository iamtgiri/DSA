...
public:
    /**
     * @brief Performs an inorder traversal of a binary tree.
    ...
    vector<int> inorderTraversal(TreeNode *root)
    {
        ...
        vector<int> In; // Create a vector to store the result.
...
private:
    /**
     * @brief A recursive helper function to perform inorder traversal.
     * @param root The current node in the traversal.
     * @param In A reference to the vector storing the traversal result.
     */
...
int main()
{
    ...
    cout << "Example 2 Inorder Traversal (empty tree): ";
    if (result2.empty())
    {
        cout << "[]";
    }
